...
def gate_result_for_action(
    ...
    *,
    ...
    """Build the state transition implied by an accept/reject action."""
    ...
    raise ValueError(
        f"unknown gate action {action!r}; expected "
        "'accept_new_best', 'accept', or 'reject'"
    )
...
def evaluate_gate(
    ...
    *,
    ...
    if cand_score > current_score:
        action: GateAction = (
            "accept_new_best" if cand_score > best_score else "accept"
        )
    else:
        action = "reject"
    return gate_result_for_action(
        action,
