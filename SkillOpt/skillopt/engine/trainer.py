...
from sage.gate import GateAction as SageAction
from sage.gate import sage_accept
...
def _coerce_selection_items(value: object) -> dict[str, float]:
    if not isinstance(value, dict):
        return {}
    out: dict[str, float] = {}
    for item_id, score in value.items():
        try:
            out[str(item_id)] = float(score)
        except (TypeError, ValueError):
            continue
    return out


def _selection_items_snapshot(items_by_hash: dict[str, dict[str, float]]) -> dict[str, dict[str, float]]:
    return {
        str(skill_id): {str(item_id): float(score) for item_id, score in items.items()}
        for skill_id, items in items_by_hash.items()
        if items
    }
...
class ReflACTTrainer:
    ...
    def train(self) -> dict:
        ...
        def _persist_runtime_state(last_completed_step: int) -> None:
            _save_runtime_state(
                out_root,
                {
                    ...
                    "selection_items_by_hash": _selection_items_snapshot(sel_per_item),
                },
            )

        # ── Selection cache ──────────────────────────────────────────────
        sel_cache: dict[str, tuple[float, float]] = {}
        sel_per_item: dict[str, dict[str, float]] = {}
        for rec in history:
            sh = rec.get("candidate_hash", "")
            if sh and rec.get("selection_hard") is not None:
                sel_cache[sh] = (rec["selection_hard"], rec["selection_soft"])
                items = _coerce_selection_items(rec.get("selection_items"))
                if items:
                    sel_per_item[sh] = items
        if runtime_state:
            saved_items = runtime_state.get("selection_items_by_hash")
            if isinstance(saved_items, dict):
                for sh, items in saved_items.items():
                    coerced = _coerce_selection_items(items)
                    if coerced:
                        sel_per_item[str(sh)] = coerced
        ...
        gate_select = str(cfg.get("gate_select", "strict")).strip().lower()
        if gate_select not in {"strict", "sage"}:
            raise ValueError("evaluation.gate_select must be 'strict' or 'sage'")
        sage_alpha = float(cfg.get("sage_alpha", 0.05))
        sage_loss_weight = float(cfg.get("sage_loss_weight", 1.0))
        sage_regression_tau = float(cfg.get("sage_regression_tau", 1.0))
        print(
            ...
            + (
                f" select=sage alpha={sage_alpha} "
                f"lambda={sage_loss_weight} tau={sage_regression_tau}"
                if gate_select == "sage"
                else " select=strict"
            )
        )
        ...
        if current_score < 0:
            ...
            sel_per_item[sh] = per_item_scores(baseline_results)
            current_origin = "initial_skill"
            best_origin = "initial_skill"
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step_in_epoch in range(steps_per_epoch):
                ...
                if cand_hash in sel_cache:
                    ...
                    sel_per_item[cand_hash] = per_item_scores(sel_results)

                step_rec["selection_hard"] = cand_hard
                step_rec["selection_soft"] = cand_soft
                if cand_hash in sel_per_item:
                    step_rec["selection_items"] = sel_per_item[cand_hash]

                cand_gate_score = select_gate_score(
                    cand_hard, cand_soft, gate_metric, gate_mixed_weight,
                )
                sage_gate_payload = None
                if gate_select == "sage":
                    current_hash = skill_hash(current_skill)
                    candidate_items = sel_per_item.get(cand_hash, {})
                    current_items = sel_per_item.get(current_hash, {})
                    if candidate_items and current_items:
                        sage_gate = sage_accept(
                            candidate_items,
                            current_items,
                            lam=sage_loss_weight,
                            tau=sage_regression_tau,
                            alpha=sage_alpha,
                        )
                        action = gate_action_from_sage(
                            commit=sage_gate.action == SageAction.COMMIT,
                            beats_best=cand_gate_score > best_score,
                        )
                        gate = gate_result_for_action(
                            action,
                            candidate_skill=candidate_skill,
                            cand_score=cand_gate_score,
                        ...
                        sage_gate_payload = {
                            "action": sage_gate.action.value,
                            "paired_n": sage_gate.ledger.n,
                            "wins": sage_gate.ledger.wins,
                            "losses": sage_gate.ledger.losses,
                            "ties": sage_gate.ledger.ties,
                            "p_value": sage_gate.p_value,
                            "discounted_gain": sage_gate.discounted_gain,
                            "regression_rate": sage_gate.regression_rate,
                            "alpha": sage_alpha,
                            "loss_weight": sage_loss_weight,
                            "regression_tau": sage_regression_tau,
                        }
                    else:
                        gate = evaluate_gate(
                            candidate_skill=candidate_skill,
                ...
                step_rec["gate_select"] = gate_select
                step_rec["candidate_gate_score"] = cand_gate_score
                if sage_gate_payload is not None:
                    step_rec["sage_gate"] = sage_gate_payload
                    ...
                    if slow_result and slow_result.get("slow_update_content"):
                        ...
                        if slow_gate_with_selection:
                            ...
                            if slow_candidate_hash in sel_cache:
                                ...
                                sel_per_item[slow_candidate_hash] = per_item_scores(
                                    slow_eval_results
                                )

                            slow_gate_score = select_gate_score(
                                slow_sel_hard,
                                slow_sel_soft,
                                gate_metric,
                                gate_mixed_weight,
                            )
                            slow_sage_payload = None
                            if gate_select == "sage":
                                current_hash = skill_hash(current_skill)
                                candidate_items = sel_per_item.get(
                                    slow_candidate_hash, {}
                                )
                                current_items = sel_per_item.get(current_hash, {})
                                if candidate_items and current_items:
                                    ...
                                    action = gate_action_from_sage(
                                        commit=sage_gate.action == SageAction.COMMIT,
                                        beats_best=slow_gate_score > best_score,
                                    )
                                    slow_gate = gate_result_for_action(
                                        action,
                                        candidate_skill=slow_candidate,
                                        cand_score=slow_gate_score,
                                        current_skill=current_skill,
                                        current_score=current_score,
                                        best_skill=best_skill,
                                        best_score=best_score,
                            ...
                            slow_result["gate_select"] = gate_select
                            if slow_candidate_hash in sel_per_item:
                                slow_result["selection_items"] = sel_per_item[
                                    slow_candidate_hash
                                ]
                            if slow_sage_payload is not None:
                                slow_result["sage_gate"] = slow_sage_payload
                            ...
                            prev_current_hash = skill_hash(current_skill)
                            prev_best_hash = skill_hash(best_skill)
                            current_skill = replace_slow_update_field(
                            ...
                            if prev_current_hash in sel_per_item:
                                sel_per_item[slow_candidate_hash] = sel_per_item[
                                    prev_current_hash
                                ]
                            best_slow_hash = skill_hash(best_skill)
                            if prev_best_hash in sel_per_item:
                                sel_per_item[best_slow_hash] = sel_per_item[
                                    prev_best_hash
                                ]
